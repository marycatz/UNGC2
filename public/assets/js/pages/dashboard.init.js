...
$('#companysize').on('select2:select', function (e) {
  ...
  var valuedata1industry = data1industry[0].text; // alert(valuedata1companysize);
...
$('#industry').on('select2:select', function (e) {
  ...
  if (valuedata != "All Industry") {
    element2.style.display = 'none'; // element1.style.display = 'none';
  ...
  Swal.fire({
    ...
    $.ajax({
      ...
      success: function success(response) {
        Swal.fire('Remind!', 'User deleted successfully!', 'success'); // get_company_data()
      }
    });
  });
}), $(".select2").select2();
...
options = {
  ...
  colors: ['#FFA500', '#F7B844', '#ADD8C7', '#EC3C65', '#CDD7B6', '#C1F666', '#D43F97', '#1E5D8C', '#421243', '#7F94B0', '#EF6537', '#C0ADDB'],
  plotOptions: {
    treemap: {
      distributed: true,
      enableShades: false
